import { type HypaModel, localModels, bulkSetPersistedHypaVectors, bulkGetPersistedHypaVectors, contextHash, truncateErrorBody } from "./hypamemory";
...
export class HypaProcessorV2<TMetadata> {
  ...
  private async getEmbeds(
    ...
    // Load persistent vectors in one request instead of one request per item.
    ...
    if (needsCacheLookup.length > 0) {
      ...
      for (let i = 0; i < needsCacheLookup.length; i++) {
        ...
        const cached = cachedVectors.get(cacheKeys[i]) as EmbeddingResult<TMetadata> | undefined;
